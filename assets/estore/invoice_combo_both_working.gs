...
function generateInvoicePDFBlob(data) {
  ...
  Object.keys(staticFields).forEach(key => {
    body.replaceText(`{{${key}}}`, staticFields[key]);
  });
  ...
  if (productTable && placeholderRowIndex > -1) {
    ...
    data.items.forEach((item, i) => {
      ...
      for (let j = 0; j < templateRow.getNumCells(); j++) {
        const cell = templateRow.getCell(j);
        const newCell = newRow.appendTableCell(cell.getText());
        newCell.setAttributes(cell.getAttributes());
      }
      newRow.getCell(0).setText(String(i + 1));
      newRow.getCell(1).setText(item.name);
      newRow.getCell(2).setText("1");
      newRow.getCell(3).setText(`₹${item.price}`);
    });
